...
import { GROUPS, COLORS } from "../lib/constants";
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-20" 
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex justify-between items-center sticky top-20 z-10">
          <button onClick={() => { const idx = GROUPS.indexOf(activeTab); if(idx > 0) setActiveTab(GROUPS[idx-1]); }} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:bg-slate-100 transition-colors font-bold">←</button>
          ...
          <button onClick={() => { const idx = GROUPS.indexOf(activeTab); if(idx < GROUPS.length - 1) setActiveTab(GROUPS[idx+1]); else setActiveTab("SUMMARY"); }} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:bg-slate-100 transition-colors font-bold">→</button>
      </div>
      ...
      <div className="space-y-4">
          {currentGroupMatches.map((m: any) => {
             ...
             return (
                <div key={m.id} className="bg-white rounded-2xl shadow-[0_0_15px_rgba(0,0,0,0.05)] border-2 border-slate-100 overflow-hidden relative group hover:border-blue-200 transition-all">
                    ...
                    <div className="border-t border-slate-100 p-2 bg-slate-50/30 min-h-[40px] flex items-center justify-center">
                        {revealedMatches[m.id] ? (
                            ...
                            <>
                                {peekMatchId === m.id ? (
                                    <div className="flex items-center gap-2 animate-in fade-in slide-in-from-bottom-2 w-full max-w-[200px]">
                                        <select 
                                            className="flex-1 text-[10px] font-bold bg-white border border-blue-300 text-slate-700 rounded-lg py-1.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-200 shadow-sm"
                                            onChange={(e) => {
                                        ...
                                        <button onClick={() => setPeekMatchId(null)} className="w-6 h-6 flex items-center justify-center rounded-full bg-slate-200 text-slate-500 font-bold hover:bg-slate-300 transition-colors">✕</button>
                                    </div>
                                ) : (
                                    <button 
